<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="/">
        <rss xmlns:g="http://base.google.com/ns/1.0" version="2.0">
            ...
            <channel>
                <title>matfel.pl - FB</title>
                <link>https://matfel.pl/</link>
                <description>Produkty ze sklepu https://matfel.pl/</description>
                <xsl:for-each
                    select="/offer/products/product[
                            price/@gross &gt; 0 and 
                            description/name/. != '']">
                    <!-- Id towaru -->
                    <xsl:choose>
    ...
    <xsl:template name="product"
        ...
        <xsl:if test="images/icons/auction_icon/@url">
            <list_icon>
                <xsl:text disable-output-escaping="yes">&lt;![CDATA[ yes ]]&gt;</xsl:text>
            </list_icon>
        </xsl:if>
        <xsl:if test="images/icons/auction_icon/@url">
            <auction_icon>
        ...
        <xsl:if test="count(images/large/image) > 0">
            <img>
                <xsl:value-of select="count(images/large/image)" />
            </img>
        </xsl:if>
    </xsl:template>
