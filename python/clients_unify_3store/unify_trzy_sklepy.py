...
def add_record(record, source, clients, clientSources):
    ...
    if key not in clients:
        ...
        # PRIORITY będzie mapą, którą ustalimy później
...
def process_files():
    ...
    # Wybranie nagłówków do wyjścia (przyjmujemy, że rekordy mają te same klucze)
    desired_fieldnames = ["First Name", "Last Name", "Zip", "Country", "Phone", "Email"]
    
    # Transformacja rekordów do wyjściowej struktury
    output1 = [transform_record(rec) for rec in grouped["file1"]]
    output2 = [transform_record(rec) for rec in grouped["file2"]]
    output3 = [transform_record(rec) for rec in grouped["file3"]]
