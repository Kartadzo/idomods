				...
				<xsl:for-each select="/offer/products/product">
					<product id="{@id}">
						...
						<xsl:element name="price">
							<xsl:text disable-output-escaping="yes">&lt;![CDATA[</xsl:text>
							<xsl:value-of select="translate(sizes/size/price/@gross,'.',',')"/>
							<xsl:text disable-output-escaping="yes">]]&gt;</xsl:text>
						</xsl:element>
						<xsl:element name="omnibus_price_retail">
							<xsl:text disable-output-escaping="yes">&lt;![CDATA[</xsl:text>
							<xsl:value-of select="translate(iaiext:omnibus_price_retail/iaiext:site/@gross,'.',',')"/>
							<xsl:text disable-output-escaping="yes">]]&gt;</xsl:text>
						</xsl:element>
						<xsl:element name="strikethrough_retail_price">
							<xsl:text disable-output-escaping="yes">&lt;![CDATA[</xsl:text>
							<xsl:value-of select="translate(sizes/size/strikethrough_retail_price/@gross,'.',',')"/>
							<xsl:text disable-output-escaping="yes">]]&gt;</xsl:text>
						</xsl:element>
						<xsl:element name="promotion">
							<xsl:text disable-output-escaping="yes">&lt;![CDATA[</xsl:text>
							<xsl:value-of select="translate(promotion/price/normal_retail_price/iaiext:site/iaiext:price_per_size/@gross,'.',',')"/>
							<xsl:text disable-output-escaping="yes">]]&gt;</xsl:text>
						</xsl:element>
					</product>
				</xsl:for-each>
